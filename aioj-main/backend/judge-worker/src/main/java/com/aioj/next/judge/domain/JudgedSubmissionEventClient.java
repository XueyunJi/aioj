...
@Component
public class JudgedSubmissionEventClient {
    private static final Logger log = LoggerFactory.getLogger(JudgedSubmissionEventClient.class);
    private static final String INTERNAL_TOKEN_HEADER = "X-Internal-Token";

    private final RestClient restClient;

    public JudgedSubmissionEventClient(JudgeWorkerProperties properties) {
        this.restClient = RestClient.builder()
                .baseUrl(stripTrailingSlash(properties.getProblemServiceBaseUrl()))
    ...
    public boolean notifyJudgedSubmission(AiJudgedSubmissionEventRequest request) {
        ...
        try {
            restClient.post()
                    .uri("/api/v1/internal/submissions/judged-events")
            ...
            log.warn("Judged submission event rejected submission={} status={} responseStatus={}",
                    request.submissionId(), request.status(), ex.getStatusCode().value());
            return false;
        } catch (RestClientException ex) {
            log.warn("Judged submission event unavailable submission={} status={} error={}",
